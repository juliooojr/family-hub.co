...
export type FinanceBill = {
  ...
  recurrence: FinanceTransaction['recurrence']; startMonth: number; paidMonths: number[]
  notes?: string; expenseKind: 'fixed' | 'variable'
}

export type FinanceBudget = { id: string; name: string; emoji: string; limit: number }

type BillRow = {
  id: string; name: string; amount: number | string; due_day: number; category: string; responsible: string
  recurrence: FinanceTransaction['recurrence']; start_date: string; expense_kind: 'fixed' | 'variable'; notes: string | null
...
export async function getFinanceData(supabase: SupabaseClient, userId: string) {
  ...
  const budgets: FinanceBudget[] = (budgetsResult.data ?? []).map((row) => ({ id: row.id, name: row.category, emoji: row.emoji, limit: Number(row.monthly_limit) }))
