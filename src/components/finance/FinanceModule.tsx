...
const investments = [
  ['Tesouro Selic 2029', 'Renda Fixa · Família', 'R$10.000', 'R$11.240', '+12,4%', true],
  ['CDB Banco Inter 120% CDI', 'Renda Fixa · Membro', 'R$8.000', 'R$8.760', '+9,5%', true],
  ['FII HGLG11', 'FII · Família', 'R$5.000', 'R$5.480', '+9,6%', true],
  ['Previdência Privada', 'Previdência · Família', 'R$7.000', 'R$6.900', '−1,4%', false],
  ['Ações ITUB4', 'Renda Variável · Membro', 'R$2.000', 'R$1.820', '−9,0%', false],
]

export default function FinanceModule({ familyId, responsibleOptions, transactions: initialTransactions, bills: initialBills, budgets: initialBudgets, reserveGoal: initialReserveGoal }: { familyId: string; responsibleOptions: string[]; transactions: Transaction[]; bills: Bill[]; budgets: Budget[]; reserveGoal: number }) {
...
function Transactions({ transactions, categories, responsibleOptions, owner, onOwner, month, monthIndex, onMonth, onCreate, onEdit }: { transactions: Transaction[]; categories: CategoryOption[]; responsibleOptions: string[]; owner: string; onOwner: (owner: string) => void; month: string; monthIndex: number; onMonth: (value: number) => void; onCreate: () => void; onEdit: (transaction: Transaction) => void }) {
...
function Bills({ bills, responsibleOptions, owner, onOwner, month, monthIndex, pendingBillIds, onMonth, onCreate, onEdit, onToggle }: { bills: Bill[]; responsibleOptions: string[]; owner: string; onOwner: (owner: string) => void; month: string; monthIndex: number; pendingBillIds: string[]; onMonth: (value: number) => void; onCreate: () => void; onEdit: (bill: Bill) => void; onToggle: (id: string) => void }) {
...
function OwnerFilter({ owner, responsibleOptions, onOwner }: { owner: string; responsibleOptions: string[]; onOwner: (owner: string) => void }) {
  return <div className="finance-owners" aria-label="Filtrar por responsável">{responsibleOptions.map((name) => <button type="button" className={owner === name ? 'active' : ''} key={name} onClick={() => onOwner(name)}>{name}</button>)}</div>
}
...
function TransactionModal({ transaction, categories, responsibleOptions, onClose, onDelete, onSave }: { transaction: Transaction | null; categories: CategoryOption[]; responsibleOptions: string[]; onClose: () => void; onDelete: (id: string) => void; onSave: (transaction: Transaction | Transaction[]) => void }) {
...
function BillModal({ bill, categories, responsibleOptions, monthIndex, onClose, onDelete, onSave, onToggle }: { bill: Bill | null; categories: CategoryOption[]; responsibleOptions: string[]; monthIndex: number; onClose: () => void; onDelete: (id: string) => void; onSave: (bill: Bill | Bill[]) => void; onToggle: (id: string) => void }) {
  const initialCategory = categories[0]?.name ?? 'Outros'
